...
public class LocalDB extends SQLiteOpenHelper {
    ...
    String[] getComuni(String startValue, boolean provIncluded) {
        try {
            ...
            SQLiteDatabase db = getReadableDatabase();
            Cursor c = db.rawQuery("select citta.Descrizione, provincia.Sigla, INSTR(upper(citta.Descrizione),?) pos \n" +
                            "from citta inner join Provincia on provincia.id=citta.idProvincia \n" +
                            "where citta.Descrizione like ? order by pos asc limit 10",
                    new String[]{startValue.toUpperCase(), "%" + startValue + "%"});
            while (c.moveToNext()) {
