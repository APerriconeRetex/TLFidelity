...
public class menuPrincipaleActivity extends AppCompatActivity {
    ...
    static private boolean cartaSenior;
    ...
    public static boolean getCartaSenior() {
        return cartaSenior;
    }

    static public void setIdCliente(int idCliente, String codiceTessera, boolean cartaSenior) {
        menuPrincipaleActivity.idCliente = idCliente;
        menuPrincipaleActivity.codiceTessera = codiceTessera;
        menuPrincipaleActivity.cartaSenior = cartaSenior;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       try {
           long ver = ScriviVersione(this, versione);
            // Controllo di avere la versione aggiornata su db
            // la versione può essere stata aggiornata in automatico oppure manualmente
            TLFidelityWS web = new TLFidelityWS(this);
           String mac = getMacAddr(getApplicationContext());
            web.checkVersioneTabletAndUpdate(mac,String.valueOf(ver),(Integer done)->{
                String msg;
                if(done==0) {
                    msg = "Errore nel metodo del web service GetVerTabletAndUpdate";
                    msg = msg+"\nRiprovare più tardi.";
    ...
    public static long ScriviVersione(AppCompatActivity act, TextView versione) throws PackageManager.NameNotFoundException {
        PackageManager manager = act.getPackageManager();
        PackageInfo info = manager.getPackageInfo(act.getPackageName(), PackageManager.GET_ACTIVITIES);
        String mac = getMacAddr(act.getApplicationContext());
        long ver;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            ver = info.getLongVersionCode() & 0xFFFFFFFF;
        } else
            ver = (long) info.versionCode;
        versione.setText("TLCustomerApp vers. " + String.valueOf(ver)+" - "+mac);
        return ver;
    }

    public static String getMacAddr(Context applicationContext) {
        ...
        if(address.isEmpty()) {
            if(BuildConfig.DEBUG) {
                ...
                // tanto vale fare un valore casuale
                SecureRandom numberGenerator = new SecureRandom();
                byte[] macBytes = new byte[6];
                numberGenerator.nextBytes(macBytes);
                StringBuilder res1 = new StringBuilder();
                for (byte b : macBytes) {
                    if (Integer.toHexString(b & 0xFF).length() == 1) {
                        res1.append("0").append(Integer.toHexString(b & 0xFF)).append(":");
    ...
    private void ControllaAutorizzazione(TLFidelityWS web, AlertDialog dlg) {
        ...
        web.checkAutorizzazione(getMacAddr(getApplicationContext()), (Integer idPV) -> {
            if (idPV > 0) {
                ...
                if (risultato == MaiChiesto) {
                    ...
                    i.putExtra("macAddress", getMacAddr(getApplicationContext()));
                    startActivity(i);
                } else {
        ...
        Timer tmr = new Timer(60000, new Runnable() {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == ID_RICHIESTA_PERMISSION_LOCALIZZAZIONE) {
           ...
           // if (requestCode != SCRIVI_DOWNLOAD && requestCode != INSTALLA_PACCHETTI) {
            ...
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                TLFidelityWS web = new TLFidelityWS(this);
                web.checkScarico(getMacAddr(getApplicationContext()),(Boolean response)->{
                    web.insertLog(getMacAddr(getApplicationContext()), 0, menuPrincipaleActivity.class .getName(),"Sono in checkScarico per InstallaNuovaVersione(null) response -> " + String.valueOf(response),"", (Integer done)->{
                        if(done==-1) {
                            Log.e(menuPrincipaleActivity.class.getName(),"Errore checkScarico download nuova versione");
                        }
                ...
                AlertDialog.Builder builder2 = new AlertDialog.Builder(this);
                builder2.setMessage("L'applicazione non è aggiornata e non verrà avviata, abilitare il salvataggio e l'avvio dell'installazione di una nuova versione.")
        ...
        if(destination!=null)
            ...
            if (!downloaded) {
                ...
                startActivity(install);
                    ...
                    e.printStackTrace();
                    web.insertLog(getMacAddr(getApplicationContext()), 0, menuPrincipaleActivity.class .getName(),"Errore download nuova versione ","Eccezione: "+e.getMessage(), (Integer done)->{
                        if(done==-1) {
    ...
    private void ControllaApp(TLFidelityWS web, AlertDialog dlg) {
        ...
        web.insertLog(getMacAddr(getApplicationContext()), 0, menuPrincipaleActivity.class .getName(),"TLCustomerApp versione: "+ String.valueOf(ver),"", (Integer done)->{
            if(done==-1) {
        ...
        web.autorizzazioneScarico(getMacAddr(getApplicationContext()), String.valueOf(ver),(Boolean response)-> {
            if (response) {
        ...
        web.insertLog(getMacAddr(getApplicationContext()), 0, menuPrincipaleActivity.class .getName(),"IP: "+ ip,"", (Integer done)->{
            if(done==-1) {
    ...
    private void InviaPosizione() {
        ...
        editor.apply();
        TLFidelityWS web = new TLFidelityWS(this);
        web.sendPos(getMacAddr(getApplicationContext()), (float) bestLocation.getLatitude(), (float) bestLocation.getLongitude(), (Boolean v) -> {
        });
    }
